package com.danimo.user.information.insfrastructure.outputadapters.persistence;
...
import com.danimo.user.user.infrastrcture.outputadapters.persistence.UserRepositoryOutputAdapter;
...
class UserRepositoryOutputAdapterTest {
    ...
    void givenUsersExist_whenFindAllEmployees_thenReturnUsersList() {
        // Arrange
        UserDbEntity secondUserDbEntity = new UserDbEntity(UUID.randomUUID(), "jane.doe", "pass",
                "jane@example.com", "module2", true, false, true, UUID.randomUUID());

        User secondUser = new User(UUID.randomUUID(), "jane.doe", "pass", "jane@example.com",
                "module2", true, false, true, UUID.randomUUID());

        List<UserDbEntity> userDbEntities = Arrays.asList(sampleUserDbEntity, secondUserDbEntity);
        List<User> expectedUsers = Arrays.asList(sampleUser, secondUser);
        ...
        assertEquals(expectedUsers.get(0).getUsername(), result.get(0).getUsername());
        assertEquals(expectedUsers.get(1).getUsername(), result.get(1).getUsername());
